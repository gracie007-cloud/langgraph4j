...
public interface JtSelectAiModel {

    record Model( Provider provider, String name, Map<String,String> attributes ) {
        ...
        try {
            if (cloud) {
                ...
                if( model.provider()  == Model.Provider.VERTEX  ) {
                    ...
                    return Optional.of( new Model(  model.provider(),
                                                    model.name(),
                ...
                if( model.provider() == Model.Provider.GITHUB  ) {
                    return Optional.of( new Model(  model.provider(),
                                                    model.name(),
                                                    Map.of("GITHUB_MODELS_TOKEN", apikey)));
                }
                return Optional.of( new Model( model.provider(),
                                               model.name(),
                                                Map.of("OPENAI_API_KEY", apikey)));
            } else {
